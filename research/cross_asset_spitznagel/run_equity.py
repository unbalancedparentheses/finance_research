...
Compares unhedged vs hedged at various leverage and budget levels.
"""
import sys
import os
...
def main():
    ...
    es_daily = es_fut[['close', 'return']].copy()
    ...
    if has_opts:
        ...
        put_map = precompute_settlements(es_put_sels, settlement_lookup, es_fut)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'eq_equity_curves.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'eq_leverage_analysis.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    if has_opts and len(put_map) > 0:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(CHART_DIR, 'eq_put_payoffs.png'), dpi=150, bbox_inches='tight')
        plt.close()
    else:
        print('No option data - cannot analyze put economics')

    # ── 14. Cross-Asset Comparison ──
    cross_asset = {}
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'eq_drawdowns.png'), dpi=150, bbox_inches='tight')
    plt.close()
